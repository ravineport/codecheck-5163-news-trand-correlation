...
class PartsOfSpeech:
    NOUN = set(['名詞', '名詞接尾辞', '英語接尾辞', '補助名詞',
            'Alphabet', 'Kana', 'Katakana', 'Kanji', 'Roman', 'Undef',
            '括弧', '句点', '読点', '空白', 'Symbol'])
    VERB = set(['動詞語幹', '動詞活用語尾', '動詞接尾辞', '判定詞'])
    ADJECTIVE = set(['形容詞語幹', '形容詞接尾辞'])
    PRENOMINAL = set(['連体詞'])
    ADVERB = set(['連用詞'])
    CONJECTION = set(['接続詞'])
    EXCLAMATION = set(['独立詞'])
    SUFFIX = set(['接続接尾辞'])
    ARTICLE = set(['冠名詞', '冠動詞', '冠形容詞'])
    ...
    ALL_MORPHS = [NOUN, VERB, ADJECTIVE, PRENOMINAL, ADVERB, CONJECTION,
           EXCLAMATION, SUFFIX, ARTICLE, PARTICLE, NUMBER]
